/**
 * Gmail search-emails operation.
 *
 * Searches Gmail messages using Gmail query syntax (from:, to:, subject:,
 * newer_than:, etc.). Returns dual-channel IntegrationResult with compact
 * forModel (id, subject, from, snippet) and rich forUser (full metadata).
 */
...
export async function searchEmails(
  ...
  try {
    ...
    return err(new IntegrationError("Failed to parse Gmail search response"));
  }
  ...
  const result = formatListResult<EmailSummaryRich, EmailSummaryCompact, EmailSummaryRich>({
    entityName: "emails",
    items: emails,
    toModel: (item) => ({
      id: item.id,
    ...
    emptyMessage: `No emails matching "${query}" found.`,
    metadata: {
      query,
      maxResults,
